...
def mock_catalog(mocker: MockerFixture) -> None:
    ...
    dummy_catalog = DataCatalog(
        {
            "namespace.input": MemoryDataset(pd.DataFrame({"a": [1, 2, 3]})),
            "namespace.output": MemoryDataset(),
        }
    )
...
def test_caching_with_namespaced_pipeline(
    monkeypatch: MonkeyPatch,
    kedro_project_with_cache_config: Path,
) -> None:
    """Check that caching works with a namespaced pipeline."""
    # change dir
